"""
POST /chat-agents — experimental create_agent-based agents endpoint.

This route intentionally mirrors the existing /chat preprocessing pipeline
while keeping the final orchestration implementation separate. The original
endpoint remains unchanged so callers can opt into the experimental path
without risking a difficult rollback.
"""
...
def _get_agents_service(settings: Settings) -> AgentsService:
    global _agents_service
    if _agents_service is None:
        _agents_service = AgentsService(settings)
...
async def chat_agents(
    ...
    service = _get_agents_service(settings)
    try:
