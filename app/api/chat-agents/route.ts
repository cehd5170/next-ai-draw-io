...
function buildChatProxyUrl(): string | null {
    ...
    return `${baseUrl}/api/chat-agents`
}
...
export async function POST(request: Request): Promise<Response> {
    ...
    if (!upstreamUrl) {
        return Response.json(
            {
                error: "PYTHON_API_URL environment variable is required for /api/chat-agents.",
            },
            { status: 500 },
        )
    }
    ...
    try {
        ...
        return Response.json(
            {
                error: "Upstream chat-agents request failed before the stream started.",
                requestId,
            },
            { status: 502 },
        )
